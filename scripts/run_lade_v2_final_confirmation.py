...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run a locked V2 LightGBM final confirmation benchmark."
    )
    ...
    parser.add_argument("--minimum-relative-mae-improvement", type=float, default=0.01)
    parser.add_argument("--tail-mae-tolerance", type=float, default=0.02)
    ...
    if not normalization_report_path.is_file():
        raise FileNotFoundError(f"Normalization report not found: {normalization_report_path}")
    if args.minimum_relative_mae_improvement <= 0.0:
        raise ValueError("minimum-relative-mae-improvement must be positive")
    if args.tail_mae_tolerance < 0.0:
        raise ValueError("tail-mae-tolerance cannot be negative")

    cohort_integrity = json.loads(normalization_report_path.read_text(encoding="utf-8"))
    if cohort_integrity.get("status") != "PASS":
        raise RuntimeError("Final cohort normalization did not report PASS")
    disjointness = cohort_integrity.get("cohort_disjointness", {})
    if not disjointness.get("all_overlap_counts_zero", False):
        raise RuntimeError("Final cohort disjointness check did not pass")
    ...
    _, baseline_validation, _ = _fit_predict(train, validation, features=BASE_STABLE_FEATURES)
    _, v2_validation, _ = _fit_predict(train, validation, features=v2_features)
    ...
    # The model and feature suite are locked from V2 development. Validation is recorded,
    # not used to choose a candidate. Both estimators are refit on train+validation before
    # dedicated calibration and untouched final test evaluation.
    ...
    bootstrap = clustered_mae_difference_ci(
        test, y_test, baseline_test, v2_test, seed=int(args.seed)
    )
    ...
    validation_tail_guardrail = bool(
        validation_v2_tail <= validation_baseline_tail * (1.0 + float(args.tail_mae_tolerance))
    )
    final_tail_guardrail = bool(v2_tail <= baseline_tail * (1.0 + float(args.tail_mae_tolerance)))
    ci_supports_improvement = bool(float(bootstrap["ci_upper"]) < 0.0)
    effect_size_pass = bool(final_relative >= float(args.minimum_relative_mae_improvement))
    validation_signal_pass = bool(
        validation_relative >= float(args.minimum_relative_mae_improvement)
        and validation_tail_guardrail
    )
    claim_supported = bool(
        validation_signal_pass
        and effect_size_pass
        and ci_supports_improvement
        and final_tail_guardrail
        and disjointness.get("all_overlap_counts_zero", False)
    )
        ...
        [
            {
                ...
                "variant": "raw",
                **_metric_row(y_test, baseline_test_raw),
                "tail_mae": _tail_mae(y_test, baseline_test_raw, test_thresholds),
            },
            {
                "model": "lightgbm_v1_clean_baseline",
            ...
            },
            {
                ...
                "variant": "raw",
                **_metric_row(y_test, v2_test_raw),
                "tail_mae": _tail_mae(y_test, v2_test_raw, test_thresholds),
            },
            {
                "model": "lightgbm_v2_event_state",
                "variant": "median_residual_calibrated",
    ...
    scorecard.to_csv(report_dir / "v2_final_confirmation_scorecard.csv", index=False)
    ...
    validation_scorecard.to_csv(
        report_dir / "v2_final_confirmation_validation_scorecard.csv", index=False
    )

    feature_importance = pd.DataFrame(
        {"feature": v2_columns, "importance": v2_model.feature_importances_}
    ).sort_values("importance", ascending=False, kind="stable")
    feature_importance.to_csv(report_dir / "v2_final_event_feature_importance.csv", index=False)

    predictions = test[
        ["snapshot_id", "courier_id", "work_date", "query_time", "city", TARGET]
    ].copy()
    predictions["baseline_prediction_raw"] = baseline_test_raw
    predictions["baseline_prediction_calibrated"] = baseline_test
    predictions["v2_prediction_raw"] = v2_test_raw
    predictions["v2_prediction_calibrated"] = v2_test
    predictions["baseline_absolute_error"] = np.abs(y_test - baseline_test)
    predictions["v2_absolute_error"] = np.abs(y_test - v2_test)
    predictions.to_csv(report_dir / "v2_final_confirmation_test_predictions.csv", index=False)

    summary = {
        "experiment": "pre_specified_v2_event_state_final_confirmation",
        "status": "CLAIM_SUPPORTED" if claim_supported else "HOLD_BASELINE",
        "claim_supported": claim_supported,
        "claim_boundary": "independent deterministic LaDe-D whole-courier-day final confirmation; offline ETA evidence only; no production delay-prevention claim",
        "pre_registered_gate": {
            "minimum_relative_mae_improvement": float(args.minimum_relative_mae_improvement),
            "bootstrap_requirement": "upper bound of clustered challenger-minus-baseline MAE CI must be below 0",
            "tail_mae_tolerance": float(args.tail_mae_tolerance),
            "cohort_disjointness_required": True,
            "validation_signal_required": True,
        },
        "cohort_integrity": disjointness,
        "model_specification": {
            ...
            "selection": "feature suite and model settings locked from V2 development; no candidate sweep on final cohort",
