...
def _keys_from_normalized_csv(
    path: Path, *, cities: tuple[str, ...]
) -> dict[str, set[tuple[str, str]]]:
    if not path.is_file():
        raise FileNotFoundError(f"Required prior normalized cohort is missing: {path}")
    frame = pd.read_csv(path, usecols=["courier_id", "city", "work_date"])
    output = {city: set() for city in cities}
    for city, courier_id, work_date in zip(
        frame["city"].astype(str),
        frame["courier_id"].astype(str),
        ...
        if city not in output:
            continue
        raw_courier = courier_id.split("::", 1)[-1]
        parsed = pd.to_datetime(work_date, format="%Y-%m-%d", errors="coerce")
        if pd.isna(parsed):
            raise ValueError(f"Invalid work_date in {path}: {work_date}")
        ds = str(int(parsed.strftime("%m%d")))
        output[city].add((raw_courier, ds))
    return output


def _overlap_counts(
    selected: dict[str, set[tuple[str, str]]],
    other: dict[str, set[tuple[str, str]]],
    *,
    cities: tuple[str, ...],
) -> dict[str, int]:
    return {city: int(len(selected[city].intersection(other.get(city, set())))) for city in cities}


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Create a fully untouched LaDe-D V2 final-confirmation cohort."
    )
    parser.add_argument("--input-dir", type=Path, default=DEFAULT_INPUT)
    parser.add_argument("--output-csv", type=Path, required=True)
    parser.add_argument("--report-path", type=Path, required=True)
    parser.add_argument("--pilot-csv", type=Path, required=True)
    parser.add_argument("--confirmatory-csv", type=Path, required=True)
    ...
    parser.add_argument("--development-seed", type=int, default=271828)
    ...
    prior_for_development = _merge_key_sets(pilot, confirmatory)
    development = _select(
        keys_by_city,
        cities=cities,
        total=int(args.max_courier_days),
        seed=int(args.development_seed),
        exclude=prior_for_development,
    )
    prior_for_final = _merge_key_sets(pilot, confirmatory, development)
    final = _select(
        keys_by_city,
        cities=cities,
        total=int(args.max_courier_days),
        seed=int(args.seed),
        exclude=prior_for_final,
    )

    actual_pilot = _keys_from_normalized_csv(args.pilot_csv.resolve(), cities=cities)
    actual_confirmatory = _keys_from_normalized_csv(args.confirmatory_csv.resolve(), cities=cities)
    actual_development = _keys_from_normalized_csv(args.development_csv.resolve(), cities=cities)
    selected_overlap_counts = {
        "pilot": _overlap_counts(final, pilot, cities=cities),
        "confirmatory": _overlap_counts(final, confirmatory, cities=cities),
        "development": _overlap_counts(final, development, cities=cities),
    }
    normalized_overlap_counts = {
        "pilot": _overlap_counts(final, actual_pilot, cities=cities),
        "confirmatory": _overlap_counts(final, actual_confirmatory, cities=cities),
        "development": _overlap_counts(final, actual_development, cities=cities),
    }
    overlap_values = [
        value
        for mapping in [*selected_overlap_counts.values(), *normalized_overlap_counts.values()]
        for value in mapping.values()
    ]
    if any(overlap_values):
        raise RuntimeError(
            "Final cohort selection overlaps a prior cohort. "
            f"selected={selected_overlap_counts}, normalized={normalized_overlap_counts}"
        )
    ...
    report = {
        "status": "PASS",
        "purpose": "fully untouched LaDe-D V2 final-confirmation cohort",
        ...
        "sampling": {
            ...
            "final_confirmation_seed": int(args.seed),
            ...
            "excluded_selected_courier_days": {
                ...
                "development": {city: len(values) for city, values in development.items()},
            },
            "selected_courier_days_by_city": {city: len(values) for city, values in final.items()},
        },
        "cohort_disjointness": {
            "selected_overlap_counts": selected_overlap_counts,
            "normalized_overlap_counts": normalized_overlap_counts,
    ...
    print("V2_FINAL_CONFIRMATION_NORMALIZATION_COMPLETE")
